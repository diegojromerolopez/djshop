...
urlpatterns = [
	...
	url(r'^(?P<member_id>\d+)/subscribe/?$', member_views.subscribe, name="subscribe"),

	url(r'^(?P<member_id>\d+)/subscription/(?P<reference_id>\d+)/delete?$', member_views.delete_subscription, name="delete_subscription"),
